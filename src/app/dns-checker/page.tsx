...
const TYPE_COLORS: Record<string, string> = { A: "#00d4ff", AAAA: "#00ff88", MX: "#7c3aed", NS: "#f59e0b", TXT: "#00d4ff", CNAME: "#00ff88", SOA: "#94a3b8", CAA: "#ef4444", PTR: "#f59e0b" };

interface DNSAnswer { name: string; type: number; TTL: number; data: string; }
interface DNSResult { type: string; answers: DNSAnswer[]; }

async function dnsQuery(name: string, type: string): Promise<DNSResult> {
  const res = await fetch(`https://dns.google/resolve?name=${encodeURIComponent(name)}&type=${type}`);
  const d = await res.json();
  return { type, answers: d.Answer || [] };
}
...
export default function DNSChecker() {
  ...
  async function check() {
    ...
    try {
      const types = type === "ALL" ? RECORD_TYPES.slice(1) : [type.toUpperCase()];
      const all = await Promise.all(types.map((t) => dnsQuery(domain.trim(), t)));
      setResults(all);
    } catch (e: unknown) {
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 80px" }}>
      ...
      <div className="cyber-card" style={{ padding: 24, marginBottom: 24 }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <input className="cyber-input" style={{ flex: 1, minWidth: 200 }} placeholder="Enter domain (e.g. google.com)" value={domain} onChange={(e) => setDomain(e.target.value)} onKeyDown={(e) => e.key === "Enter" && check()} />
          <select value={type} onChange={(e) => setType(e.target.value)} style={{ background: "rgba(0,0,0,0.4)", border: "1px solid #1e3a5f", borderRadius: 8, color: "#e2e8f0", padding: "12px 16px", fontSize: 14, outline: "none" }}>
            {RECORD_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <button className="cyber-btn" onClick={check} disabled={loading}>{loading ? "Checking..." : "Check DNS"}</button>
        </div>
      </div>
      ...
      {loading && <div style={{ textAlign: "center", padding: 40, color: "#94a3b8" }}><div className="spinner" style={{ margin: "0 auto 16px" }} /><div>Querying DNS servers...</div></div>}

      {results && !loading && (
        <div>
          {!hasRecords && <div style={{ color: "#94a3b8", textAlign: "center", padding: 32 }}>No DNS records found.</div>}
          {results.filter((r) => r.answers.length > 0).map((result) => (
            <div key={result.type} className="cyber-card" style={{ marginBottom: 16, padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "14px 20px", borderBottom: "1px solid #1e3a5f", display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ background: TYPE_COLORS[result.type] || "#94a3b8", color: "#000", fontWeight: 800, fontSize: 11, padding: "3px 10px", borderRadius: 4 }}>{result.type}</span>
                <span style={{ color: "#94a3b8", fontSize: 13 }}>{result.answers.length} record{result.answers.length !== 1 ? "s" : ""}</span>
              </div>
              <table className="data-table">
                <thead><tr><th>Name</th><th>TTL</th><th>Data</th></tr></thead>
                <tbody>
