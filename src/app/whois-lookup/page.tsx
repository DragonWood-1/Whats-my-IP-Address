    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px 80px" }}>
      ...
      <div className="cyber-card" style={{ padding: 24, marginBottom: 24 }}>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <input className="cyber-input" style={{ flex: 1, minWidth: 200 }} placeholder="Enter domain or IP (e.g. google.com)" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && lookup()} />
          <button className="cyber-btn" onClick={lookup} disabled={loading}>{loading ? "Looking up..." : "Whois Lookup"}</button>
        </div>
      </div>
      ...
      {loading && <div style={{ textAlign: "center", padding: 40, color: "#94a3b8" }}><div className="spinner" style={{ margin: "0 auto 16px" }} /><div>Fetching registration data...</div></div>}
      ...
      {data && !loading && (
        <div style={{ display: "grid", gap: 16 }}>
          ...
          {status.length > 0 && (
            <div className="cyber-card" style={{ padding: 20 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {status.map((s) => <span key={s} className="badge badge-cyan">{s}</span>)}
              </div>
            </div>
          )}
          ...
          {events.length > 0 && (
            <div className="cyber-card" style={{ padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "16px 24px", borderBottom: "1px solid #1e3a5f" }}><h2 style={{ color: "#e2e8f0", fontSize: 16, fontWeight: 700 }}>Registration Events</h2></div>
              <table className="data-table">
                ...
                <tbody>
                  {events.map((e, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ color: "#e2e8f0", fontFamily: "monospace", fontSize: 13 }}>{new Date(e.eventDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {nameservers.length > 0 && (
            <div className="cyber-card" style={{ padding: 20 }}>
              ...
              {nameservers.map((ns) => (
                <div key={ns.ldhName} style={{ fontFamily: "monospace", color: "#00d4ff", fontSize: 14, padding: "6px 0", borderBottom: "1px solid rgba(30,58,95,0.5)" }}>{ns.ldhName}</div>
              ))}
            </div>
          )}
        </div>
