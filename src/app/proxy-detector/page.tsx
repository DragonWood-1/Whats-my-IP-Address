      ...
      {geo && !loading && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 24 }}>
            {[
              { label: "Proxy",      detected: isProxy,   icon: "🔀", desc: "Open/transparent proxy", available: !!sec },
              { label: "VPN",        detected: isVPN,     icon: "🔒", desc: "VPN service node",       available: !!sec },
              { label: "Tor",        detected: isTor,     icon: "🧅", desc: "Tor exit node",          available: !!sec },
              { label: "Datacenter", detected: isHosting, icon: "☁️", desc: "Cloud/hosting IP",       available: true  },
            ].map((item) => (
            ...
            <table className="data-table">
              <tbody>
                {[
                  ...
                  ["Risk Score",   sec ? `${riskScore}/100` : "—",                                          riskScore === 0 ? "#10b981" : riskScore <= 40 ? "#f59e0b" : "#ef4444"],
                  ["ISP",          isp],
